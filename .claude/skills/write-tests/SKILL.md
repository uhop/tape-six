...
10. **Testing HTTP code** — the package exports two subpath modules; in this repo import them via the relative path:
    ...
    - For mock-server scenarios that record requests: wrap the handler in `record(handler?)` — it buffers each request as `{method, url, headers, body}` (body eagerly read as UTF-8) onto its `requests` array, then answers 204 or delegates. A delegate reads `entry.body` (its third argument), never the already-drained `req` stream. Assert with `t.match` + `t.any`; reset between tests with `rec.requests.length = 0` in your own `beforeEach` — `setupServer` owns the lifecycle; the caller owns state.
    - Reference tests: `tests/cli/test-server.js`, `tests/cli/test-record.js`, `tests/cli/test-response.js` (under `tests/cli/` because they import `node:http` and so can't run in the browser worker; the `tape6.cli` patterns in `package.json` pick them up).
