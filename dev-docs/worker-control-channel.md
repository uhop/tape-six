...
Status: proposal (interface only, no implementation). Spans **tape-six** (the
worker abstraction + child-side listener; the in-tree `par` worker-thread,
`seq` in-process, and standalone-browser iframe transports) and **all
`tape-six-*` worker providers** that supply a transport — `tape-six-proc`
(subprocess), `tape-six-puppeteer` / `tape-six-playwright` (driver-backed
browser), and any future ones. Each provider implements the control channel for
its own transport.
...
1. **`failOnce` / bail-out should actually stop in-flight workers.** When
   ...
   stop *scheduling new files* (`fileQueue` is cleared). Already-running
...
- **Normal completion.** When the parent has consumed a task's top-level `end`,
  ...
  `end` is read *before* exit is requested.
...
2. **Drain first, kill as backstop.** `terminate` should *drain* a running
   ...
   **Caveat — drain is cooperative.** `StopTest` only lands at the *next
   assertion*, and `t.signal` only interrupts `await`s that were handed the
   ...
   Note: failOnce today throws `StopTest` but does *not* call
...
- **Layer 2 — worker/file-level deadline (new; via this channel).** The
  ...
  Layer 1 lacks: it actually stops a hung worker. It is the *same* code path as
...
| browser — iframe, driver-backed (puppeteer / playwright) | `postMessage` | driver closes page / context from Node ✓ — *not wired* |
...
the driver, not in-page. So a driver-backed browser worker *can* honor
`graceTimeout` → kill (close the page); it's simply **not wired yet**. The case
with *no* backstop is the **standalone** browser run (`tape6-server` + a human
...
Web Worker has no DOM** (`document`, etc.), so it is *not* a full browser
environment: it can run only **DOM-free** tests. That makes it a *partial*
